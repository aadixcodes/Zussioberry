...
<section
      ...
      >
        <div className="relative max-w-6xl mx-auto w-full px-4 py-10 sm:py-14 md:py-16 flex flex-col-reverse md:grid md:grid-cols-2 gap-10 items-center h-full">
          {/* Text Content */}
          <div className="w-full flex flex-col items-center md:items-start text-center md:text-left mt-8 md:mt-0 justify-center">
            ...
            <p className="text-xs sm:text-sm md:text-lg font-[para] text-gray-200 leading-relaxed mb-5 sm:mb-6 max-w-xl">
              {firstBlog.description.length > 200 ? `${firstBlog.description.substring(0, 200)}...` : firstBlog.description}
            </p>
          ...
          <div className="w-full flex justify-center md:justify-end items-center">
            <Image
              src={firstBlog.image || "/assets/Blogs.png"} // Fallback image
              alt={firstBlog.title}
              width={320}
              height={320}
              className="rounded-lg shadow-lg w-60 h-60 sm:w-80 sm:h-80 object-cover transition-transform duration-500 hover:scale-105"
              priority
            />
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 space-y-10">
              {blogsData.slice(1).map((blog) => (
                <div
                  ...
                  <Image
                    src={blog.image || "/assets/Blogs.png"} // Fallback image
                    alt={blog.title}
                    width={600} // Increased size
                    height={300} // Increased size
                    className="w-full h-85 object-cover transition-transform duration-500 hover:scale-105" // Increased height
                  />
                  <div className="p-6">
                    ...
                    <p className="text-gray-600 mt-2 font-[para]">{blog.description}</p>
            ...
            <div className="relative">
              <aside className="bg-gray-50 p-6 rounded-lg shadow-md self-start sticky top-24 h-fit max-h-[80vh] overflow-y-auto">
                <h4 className="text-xl font-[para] mb-6">Recent Posts</h4>
                <div className="space-y-4">
                  {recentPosts.map((post) => (
                    <div key={post.id} className="flex gap-4 items-center transition-transform duration-300 hover:translate-x-2">
                      <Image
                        src={post.image || "/assets/Blogs.png"} // Fallback image
                        alt={post.title}
                        width={70}
                        height={70}
                        className="rounded-md transition-transform duration-500 hover:scale-110 object-cover"
                      />
                      <div>
